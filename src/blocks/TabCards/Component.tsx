...
export const TabCards: React.FC<TabCardsProps> = ({ tabs, color }) => {
    ...
    <div className="container max-w-4xl overflow-hidden">
      <div className="bg-slate-100 rounded-lg shadow-md">
        ...
        <div className="text-center py-4 text-gray-700">
          {(tabs && tabs.length && tabs[activeTab] && (
            <RichText className="max-w-[100%] dark:text-black" data={tabs[activeTab].text} />
          )) || <label>Please define some tabs</label>}
        </div>
      </div>
    </div>
  )
